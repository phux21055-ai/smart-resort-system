...
const BookingCalendar: React.FC = () => {
    ...
    try {
      ...
      if (result.success) {
        const formattedEvents = result.data.map((booking: any) => ({
          ...
          backgroundColor: getStatusColor(booking.confirmationStatus),
          extendedProps: {
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s === 'confirmed' || s === 'ยืนยันแล้ว') return '#10b981'; // Green
    if (s === 'waiting' || s === 'รอการยืนยัน' || s === 'pending') return '#f59e0b'; // Orange
    if (s === 'cancelled' || s === 'ยกเลิก') return '#ef4444'; // Red
